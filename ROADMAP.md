...
| 12      | **ART-012 Structured AI contract**  | Versioned Zod contract разделяет facts/inferences, содержит source IDs, confidence, model/prompt/schema versions; невалидный ответ не получает успешный статус. Негативные contract tests обязательны.   | ART-003, ART-011 | `NEXT` |
