...
| 13      | **ART-013 Full offline pipeline**   | Синхронный application orchestrator и команда `process:fixtures` проводят материал через persistence, rules, fake analysis и scoring. Повторный запуск идемпотентен и печатает счётчики стадий.          | ART-004–012      | `NEXT` |
