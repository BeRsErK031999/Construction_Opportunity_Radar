...
| 4       | **ART-004 PostgreSQL persistence** | Docker Compose, Prisma migrations, repositories, integration tests и development seed. После чистого seed: 10 sources, 100 raw items, 0 signals.                                                                                                                              | ART-003          | `NEXT`    |
| 5       | **ART-005 Fixture source adapter** | `SourceAdapter` и `FixtureSourceAdapter`; 100–200 версионированных материалов Construction/HoReCa/other с рекламой, exact- и near-дублями; ingestion идемпотентен и проверяет permission boundary.                                                                            | ART-003, ART-004 | `PLANNED` |
...
| Порядок | Задача                                              | Результат и проверяемый выход                                                                                                                                                                                                                                             | Зависимости          | Статус    |
| ------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | --------- |
| 6       | **ART-007 Normalization pipeline**                  | HTML/пробелы/повторяющиеся строки очищаются, даты и язык нормализуются, canonical URL и versioned normalized representation сохраняются; raw content не изменяется и не удаляется.                                                                                        | ART-003–005          | `PLANNED` |
| 7       | **ART-008 Exact and near deduplication**            | Exact match по source identity, canonical URL и normalized hash; bounded near-match по вертикали и временному окну; false-positive fixtures и метрики. Целевой fixture-run показывает примерно 150 уникальных из 200, но фактические числа фиксируются тестовыми данными. | ART-007              | `PLANNED` |
| 8       | **ART-006 RSS/HTTP adapter**                        | Generic RSS и/или public HTTP adapter с timeout, bounded retry, rate limit, user-agent, provenance, timestamps и безопасными ошибками. Fixture tests не зависят от сети; live smoke разрешён только для одобренного источника.                                            | ART-005, ART-007–008 | `PLANNED` |
| 9       | **ART-009 Vertical classifier and relevance rules** | Детерминированный baseline `CONSTRUCTION`, `HORECA`, `OTHER` по source metadata, словарям и правилам; AI получает только разрешённые и потенциально релевантные уникальные материалы.                                                                                     | ART-007–008          | `PLANNED` |
| 10      | **ART-010 Opportunity scoring**                     | Чистая versioned функция пяти факторов 0–100 с breakdown и порогами; `companyFit` вычисляется для профиля, а не хранится как глобальное свойство сигнала. Unit tests проверяют веса, границы и объяснение.                                                                | ART-003, ART-009     | `PLANNED` |

Milestone M1: 200 fixtures проходят `raw -> normalized -> deduplicated -> classified`, статистика воспроизводима, raw сохраняется, AI не вызывается.

## M2 — независимый от модели pipeline

| Порядок | Задача                              | Результат и проверяемый выход                                                                                                                                                                            | Зависимости      | Статус    |
| ------- | ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- | --------- |
| 11      | **ART-011 AI provider abstraction** | Порт `AIProvider` (`analyzeSignal`, `healthCheck`, `modelInfo`) не содержит Ollama-специфики; `FakeAIProvider` детерминированно возвращает валидный анализ и поддерживает управляемые ошибки для тестов. | ART-003, ART-010 | `PLANNED` |
| 12      | **ART-012 Structured AI contract**  | Versioned Zod contract разделяет facts/inferences, содержит source IDs, confidence, model/prompt/schema versions; невалидный ответ не получает успешный статус. Негативные contract tests обязательны.   | ART-003, ART-011 | `PLANNED` |
| 13      | **ART-013 Full offline pipeline**   | Синхронный application orchestrator и команда `process:fixtures` проводят материал через persistence, rules, fake analysis и scoring. Повторный запуск идемпотентен и печатает счётчики стадий.          | ART-004–012      | `PLANNED` |

Milestone M2: воспроизводимый end-to-end run с `AI provider: fake`; это точка допуска к продуктовым интерфейсам. Gate G2 ещё не считается пройденным без эксплуатационных метрик и restart evidence.

## M3 — API, Telegram и feedback loop

| Порядок | Задача                                 | Результат и проверяемый выход                                                                                                                                                                                 | Зависимости               | Статус    |
| ------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | --------- |
| 14      | **ART-014 Application API**            | Health, signals, sources, user profile и feedback endpoints; Zod validation, фильтры, authorization boundary и safe errors. Source Registry API не разрешает AI processing без прав.                          | ART-004, ART-013          | `PLANNED` |
| 15      | **ART-015 Telegram UI**                | Меню возможностей, дайджеста, сохранённых, интересов и помощи; карточка показывает score, объяснение, действия и source link. Transport adapter тестируется без реального токена.                             | ART-014                   | `PLANNED` |
