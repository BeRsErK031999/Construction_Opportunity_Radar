...
| 9       | **ART-009 Vertical classifier and relevance rules** | Активный `classifier-v2` детерминированно классифицирует только dedup-кластеры с разрешённым evidence и закрывает pilot false-negative для русских форм завершённого строительства; v1 сохранён. Fixture-run остаётся 150 решений, 110 AI-eligible, 28 irrelevant и 12 permission-denied; повтор создаёт 0 signals.                                                           | ART-007–008          | `DONE` |
| 10      | **ART-010 Opportunity scoring**                     | Активный `opportunity-score-v2` сохраняет формулу 35/25/20/10/10, но ограничивает confidence надёжностью fact sources и применяет versioned priority cap; weak evidence больше не становится HIGH. Company Fit и exclusions остаются детерминированными, v1 сохранён, explanation содержит raw score и guardrail evidence.                                               | ART-003, ART-009     | `DONE` |
...
| 15      | **ART-015 Telegram UI**                | grammY long-polling bot реализует точное меню из пяти пунктов, private-chat boundary, профиль/помощь, новые и сохранённые карточки со score, объяснением, 2–3 приоритетными действиями и source link. `DeliveryPort`, fake/Telegram adapters, persisted `PENDING/SENT/FAILED`, replay idempotency и кнопки `USEFUL/NOT_USEFUL/SAVED` проверяются без токена; live smoke явно не выполнялся.                                     | ART-014                   | `DONE`    |
| 16      | **ART-016 Feedback loop**              | Все пять действий `USEFUL/NOT_USEFUL/SAVED/ACTED/ALREADY_KNOWN` доступны через Telegram и versioned HTTP contract; optional reason, user/recommendation/delivery/correlation attribution, sentiment conflict и concurrent idempotency сохраняются в PostgreSQL. Персональный read model считает coverage, positive sentiment, direct/Telegram attribution и `HIGH/CRITICAL + NOT_USEFUL` без автоматического изменения scoring. | ART-014–015               | `DONE`    |
| 17      | **ART-017 Digest**                     | `digest-v1` сохраняет персональные UTC daily top-5 и weekly summary из compact Recommendation: детерминированный score/time/id порядок, version/profile/period identity, source links и метрики processed/unique/relevant/opportunities/HIGH с ростом категорий. Telegram menu доставляет daily через отдельный `PENDING/SENT/FAILED`; повтор создаёт 0 Digest и 0 transport calls.                                                         | ART-010, ART-014–016      | `DONE`    |
| 18      | **ART-018 Durable jobs and scheduler** | PostgreSQL `processing_jobs` и worker/scheduler runtime поддерживают семь stage types, versioned payload/correlation, deterministic time buckets, atomic `SKIP LOCKED` claim, active concurrency key, owner lease/renewal, bounded exponential retry, stale recovery и terminal failure. Unit и 14 PostgreSQL integration tests подтверждают concurrent single-claim, restart без duplicate job и исчерпание attempt budget. | ART-004, ART-013, ART-017 | `DONE`    |

Milestone M3 достигнут: пользовательский путь работает с fake Telegram transport, а durable runtime безопасно планирует и восстанавливает pipeline/digest jobs. Live Telegram smoke относится к внешней проверке credentials.

## M4 — evals, эксплуатация и контракт интеграции

| Порядок | Задача                                           | Результат и проверяемый выход                                                                                                                                                                                                     | Зависимости                   | Статус     |
| ------- | ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | ---------- |
| 19      | **ART-019 Eval dataset**                         | `eval-gold/v1` содержит 200 project-authored synthetic материалов: по 100 Construction и HoReCa, по 80 relevant и 20 negative; размечены category/event type, 360 evidence-backed facts, summary, expected action и importance. Строгий contract, loader/validator, 80 calibration/120 holdout и tests отделяют gold set от ingestion fixtures. Baseline прозрачно не считается human-adjudicated. | ART-003, ART-005, ART-009–012 | `DONE`     |
| 20      | **ART-020 AI benchmark harness**                 | `benchmark:ai` принимает provider/model/dataset/split и выдаёт strict `ai-benchmark-report/v1`: structured validity/coverage, event type и relevance confusion metrics, source-supported factual errors/hallucination count, p50/p95 latency, optional real token/VRAM telemetry и typed failures. Fake-прогон 200/200 проверяет harness без label leakage; 8B/14B comparison остаётся внешним evidence. | ART-011–012, ART-019          | `DONE`     |
| 21      | **ART-021 Observability**                        | Application/job typed observers объединяют ingestion, pipeline, AI, delivery и durable jobs со stable `snake_case` JSON events, correlation/source/run context и bounded-cardinality `radar_metrics/v1` counters. Offline CLI возвращает snapshot, bot/worker логируют финальный snapshot; observer fail-open, payload/PII/secret не входят в контракт. Grafana/exporter остаются deployment-решением. | ART-002, ART-013, ART-018     | `DONE`     |
| 22      | **ART-022 Security hardening**                   | Candidate/history secret scan и production dependency audit воспроизводимы; private API разделяет admin/user credentials, ограничивает IP rate/body и безопасно логирует route; logger санитизирует credentials в errors. HTTP collector блокирует private/reserved targets и повторно проверяет redirects. PostgreSQL runtime role имеет DML без DDL/owner-прав; threat baseline и rotation/egress runbook фиксируют остаточные риски. | ART-014–015, ART-021          | `DONE`     |
| 23      | **ART-023 Backup and restore**                   | `db:backup` стримит PostgreSQL custom dump в authenticated AES-256-GCM `.corbak`, разделяет environment storage и bounded retention; `db:verify-backup` реально восстанавливает во временную БД, проверяет schema/counts и удаляет только её. `db:restore` создаёт новую явно названную БД без overwrite. Runbook фиксирует key custody, off-host copy, rotation и G5 schedule evidence. Проверены valid/wrong-key и named restore paths. | ART-004, ART-018, ART-022     | `DONE`     |
| 24      | **ART-024 CI**                                   | Workflow с immutable action/image pins разделяет полный quality-набор и PostgreSQL integration: frozen install, format, lint, typecheck, Prisma/eval validation, unit tests, build, secret/audit checks и 14 migration-backed integration tests. Telegram, Ollama и production credentials отсутствуют; внешний DB override ограничен loopback `radar_test`. GitHub run `33722329752` прошёл оба job. | ART-002–023                   | `DONE`     |
| 25      | **ART-025 Denis-PC/Ollama integration contract** | `OllamaAIProvider` использует non-streaming structured `/api/chat`, общий `ai-analysis/v1` mapper, bounded input/output/timeouts/concurrency и benchmark token telemetry. Provider-discriminated config оставляет fake default, запрещает небезопасный remote origin и требует точную model identity; `/api/tags` health, ADR-0006 и host runbook проверены offline без Ollama. Реальные 8B/14B smoke, VRAM и full benchmark остаются внешним evidence. | ART-011–012, ART-020–022      | `DONE`     |

Milestone M4 достигнут локально: кодовая база готова к безопасному подключению inference-host. Gate G1 требует реальных одинаковых прогонов 8B и 14B; Gate G5 требует эксплуатационных данных, backup/restore и reboot evidence.
