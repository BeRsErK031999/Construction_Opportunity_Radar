...
| Поле                                       | Тип                      | Смысл                                                                                                               |
| ------------------------------------------ | ------------------------ | ------------------------------------------------------------------------------------------------------------------- |
| `userId`                                   | UUID                     | Владелец персональной сводки                                                                                        |
| `generatedAt`                              | datetime                 | Время построения read model                                                                                         |
| `actions`                                  | object                   | All-time counts: `useful`, `notUseful`, `saved`, `acted`, `alreadyKnown`                                            |
| `attribution`                              | object                   | Количество `direct` HTTP и `telegram` действий; сумма равна `totals.actions`                                        |
| `totals.actions`                           | non-negative integer     | Сумма пяти action counts                                                                                            |
| `totals.recommendationsWithFeedback`       | non-negative integer     | Уникальные Recommendation хотя бы с одним action                                                                    |
| `totals.deliveredRecommendations`          | non-negative integer     | Уникальные Recommendation с успешным Delivery                                                                       |
| `totals.evaluatedDeliveredRecommendations` | non-negative integer     | Уникальные успешно доставленные Recommendation хотя бы с одним action                                               |
| `feedbackCoveragePercent`                  | number `0..100`          | `evaluatedDeliveredRecommendations / deliveredRecommendations * 100`, два знака; при нулевом знаменателе `0`        |
| `positiveSentimentPercent`                 | number `0..100` или null | `useful / (useful + notUseful) * 100`, два знака; `null`, пока sentiment отсутствует                                |
| `highScoreNotUseful[]`                     | object[]                 | До запрошенного limit: `HIGH/CRITICAL + NOT_USEFUL`, score/headline/reason, direct/Telegram attribution и trace IDs |

Сводка не меняет веса или статус Recommendation. Период и cohort не принимаются в v1: значения all-time, чтобы не вводить неоднозначную связь между временем Delivery и более поздним Feedback.
