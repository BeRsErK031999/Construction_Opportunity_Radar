...
The active `classifier-v2` consumes one `deduplicator-v1` cluster at a time. Permission is checked before text rules: if the dedup representative is not AI-permitted, the earliest permitted member becomes the classification input; if no member is permitted, the outcome is `PERMISSION_DENIED` and no text is exposed as AI input. An eligible decision exposes only permitted cluster members, while the dedup table retains the full evidence graph. V2 adds explicit Russian completed-construction verb forms after a manual pilot exposed a v1 false negative; `classifier-v1` remains immutable for historical reproduction.

The classifier combines one-point source vertical hints with explicit Construction/HoReCa dictionaries, then applies advertisement, explicit-negative, ambiguity, and opportunity-cue rules. Outcomes are `AI_ELIGIBLE`, `IRRELEVANT`, or `PERMISSION_DENIED`; every decision carries stable reason/rule IDs, vertical scores, `classifier-v2`, and `signal-taxonomy-v1`. Only `AI_ELIGIBLE` decisions become `CANDIDATE` signals. Signal identity includes the dedup representative and all three policy versions; matched rules and permitted provenance links are saved atomically.
...
`opportunity-score-v2` retains the v1 weights and inclusive bands: `IGNORE < 40`, `LOW >= 40`, `MEDIUM >= 55`, `HIGH >= 70`, `CRITICAL >= 85`. It derives evidence reliability conservatively from fact-source attribution, uses `min(analysis confidence, evidence reliability)` as the persisted confidence factor, and then applies display-safe maximum scores of `54/69/84/100` for effective-confidence ranges `<40/<60/<80/80+`. The explanation includes the raw weighted score and any cap. This prevents a weakly supported high-impact item from becoming `HIGH` or `CRITICAL`; details and pilot evidence are in ADR-0007. `opportunity-score-v1` remains immutable for reproduction.

Current API and bot opportunity queries select only the active `classifier-v2` + `opportunity-score-v2` pair. Historical v1 recommendations remain stored for audit, saved-card history, and feedback attribution instead of being silently rewritten or mixed into current rankings.
