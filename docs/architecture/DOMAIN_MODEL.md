...
Статус: реализовано в `ART-003`, 2026-09-01.
...
| ------------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Source`            | Политика разрешённого источника, сбор и routing                | AI разрешён только для `OPEN_DATA`, `PUBLIC_API`, `PARTNER`, `CONSENT` с документированным basis; Telegram-источник требует `PARTNER`/`CONSENT`; live-источник имеет положительный poll interval.                                                              |
...
| `NormalizedItem` | `normalized_items`; entities `jsonb`                                                                                                                                      | PK `id`; FK raw item; unique `(raw_item_id, normalizer_version)`; index normalized hash, canonical URL и publication time. Rejected attempt станет отдельным operational record в ART-007, а не частично валидным `NormalizedItem`. |
| `Signal`         | `signals` плюс `signal_items(signal_id, normalized_item_id, source_id)`                                                                                                   | PK signal; unique link rows; FK до normalized/source; checks scores/status; partial check/FK для superseded relation; indexes vertical/category/status/time.                                                                        |
...
- Prisma schema, migrations, repository interfaces и integration tests относятся к `ART-004`.
- Rejection/failure lifecycle нормализации, dedup evidence и classifier rules уточняются в `ART-007`–`ART-009`.
