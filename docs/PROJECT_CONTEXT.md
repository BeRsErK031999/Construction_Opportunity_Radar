...
- `ART-020 AI benchmark harness` is next. Gate G1 remains open until the harness and identical external 8B/14B runs exist. Interactive onboarding/profile editing and concrete production schedules remain deferred.
...
- Current evidence: frozen install, format, lint, strict typecheck, 184 unit/contract/HTTP/bot/job/eval tests, build, Prisma generation/validation, 14 PostgreSQL integration tests, dependency audit, deterministic eval validation, and two full local CLI runs succeed. The Docker-backed suite covers the idempotent 200 -> 200 -> 150 -> 110 -> 110 -> 110 chain plus API, daily/weekly Digest, Telegram delivery, five feedback outcomes, concurrent idempotency, composite attribution integrity, feedback summary, job single-claim/retry/stale/terminal paths, and restart recovery.
- The ART-001–ART-018 implementation is pushed to `origin/main` through commit `84bcd93`; ART-019 changes are local until the next explicit push request. No deploy, live source call, Telegram call, or Ollama call has been performed.
