...
- `ART-023 Backup and restore` is next. Gate G1 remains open until identical external 8B/14B runs exist; fake metrics are functional evidence only. Interactive onboarding/profile editing and concrete production schedules remain deferred.
...
- ART-001–ART-021 are pushed to `origin/main` through commit `221f944`; ART-022 changes remain local until an explicit commit/push request. No deploy, live source call, Telegram call, or Ollama call has been performed.
