...
- `ART-009 Vertical classifier and relevance rules` is complete: the active `classifier-v2` selects one permitted representative per dedup cluster, classifies Construction/HoReCa/OTHER from versioned source/text rules, recognizes Russian completed-construction forms, rejects advertisements/no-opportunity/ambiguous material, and exposes only permitted evidence to future AI work. V1 remains immutable for reproduction.
...
- `ART-010 Opportunity scoring` is complete: active `opportunity-score-v2` retains the 35/25/20/10/10 formula and explicit band thresholds, but effective confidence is now the lower of AI confidence and fact-source reliability; a versioned cap prevents insufficiently supported material from reaching a misleading priority band. V1 remains immutable for reproduction.
...
- A 2026-09-03 manual Codex pilot over five current materials produced three AI-eligible signals, one irrelevant decision and one permission denial. After v2 corrections, the official high-confidence item remained `HIGH 80.75`, a retrospective construction summary became `LOW 54.75`, and an unreviewed `35/100` source moved from `HIGH 72.95` to `LOW 54`; an unchanged repeat created no rows, AI calls or deliveries.
...
- `ART-024 CI` is complete as two least-privilege GitHub Actions jobs with immutable action/image pins. `Quality` uses full history, frozen pnpm install, format/lint/typecheck, Prisma/eval validation, unit tests, build, secret scan and production audit; `PostgreSQL integration` uses only an ephemeral PostgreSQL service and 14 migration-backed tests.
