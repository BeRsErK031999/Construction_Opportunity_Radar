...
- `ART-018 Durable jobs and scheduler` is next. Interactive onboarding/profile editing and automatic digest scheduling remain deferred.
...
- Current evidence: frozen install, format, lint, strict typecheck, 169 unit/contract/HTTP/bot tests, build, Prisma generation/validation, 10 PostgreSQL integration tests, dependency audit, and two full local CLI runs succeed. The Docker-backed suite covers the idempotent 200 -> 200 -> 150 -> 110 -> 110 -> 110 chain plus API, daily/weekly Digest, Telegram delivery, five feedback outcomes, concurrent idempotency, composite attribution integrity, and feedback summary paths.
- The ART-001–ART-016 implementation is committed and pushed to `origin/main` through commit `b4ee4d3`; ART-017 changes are local until the next explicit push request. No deploy, live source call, Telegram call, or Ollama call has been performed.
