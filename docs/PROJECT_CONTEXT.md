...
- `ART-022 Security hardening` is next. Gate G1 remains open until identical external 8B/14B runs exist; fake metrics are functional evidence only. Interactive onboarding/profile editing and concrete production schedules remain deferred.
...
- Current evidence: frozen install, format, lint, strict typecheck, 194 unit/contract/HTTP/bot/job/eval/observability tests, build, Prisma generation/validation, 14 PostgreSQL integration tests, dependency audit, deterministic eval validation, one full fake benchmark, and repeat local CLI pipeline runs succeed. The Docker-backed suite covers the idempotent 200 -> 200 -> 150 -> 110 -> 110 -> 110 chain plus API, daily/weekly Digest, Telegram delivery, five feedback outcomes, concurrent idempotency, composite attribution integrity, feedback summary, job single-claim/retry/stale/terminal paths, restart recovery, and telemetry propagation across the complete pipeline.
- The ART-001–ART-020 implementation is pushed to `origin/main` through commit `afb4024`; ART-021 changes are local until the next explicit push request. No deploy, live source call, Telegram call, or Ollama call has been performed.
